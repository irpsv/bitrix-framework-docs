...
Минимальная конфигурация должна включать в себя хост и логин:
```php
'smtp' => [
	'value' => [
		'enabled' => true,
...
Дополнительно можно указать параметры авторизации и нестандартный порт подключения:
```php
'smtp' => [
	'value' => [
		'enabled' => true,
		...
		'login' => 'user',
		'password' => '123456',
		// в случае использования OAuth авторизации
		'isOauth' => true,
	],
	'readonly' => true,
],
...
Также поддерживаются дополнительные параметры:
```php
'smtp' => [
	'value' => [
		// ...
		...
		'encryption_type' => 'smtps', // в случае указания 465 порта будет использоваться `ssl`, иначе `starttls`
	],
	'readonly' => true,
],
```

Для отладки достаточно включить опцию `debug` и опционально указать файл лога:
```php
'smtp' => [
	'value' => [
		// ...
...
В случае если `logFile` не указан, то будет создан `mailer.log` файл в корне сайта (DOCUMENT_ROOT).

{% note info "" %}

Более гибкие и широкие настройки серверов отправителей выполняется через административный интерфейсе на странице *Настройки > Настройки продукта > Почтовые и СМС события > Настройки SMTP*.
